	  ...
	  <input type="hidden" name="thispage" value="<?=$thispage?>" />
       <table cellpadding="0" cellspacing="0" class="biaozhun">
        <tr>
          ...
          <td width="42%" class="cknr2"><input type="text" class="bzsr" name="industry" value="<?=$industry?>"/></td>
        </tr>
        <tr>
          <td colspan="2">
		    <table cellspacing="0" cellpadding="0" class="biaozhun2">
              ...
              <?php 
              if($department){
              	 $dpt = explode(';',$department);
              	 $telephone = explode(';',$tel);
              	 foreach($dpt as $k=>$v){
              	 	?>
              	<tr>
                <td>
                     <input type="text" class="bzsr" name="department[]" value="<?=$v?>" />
                </td>
                <td>
                    <input type="text" class="bzsr" name="tel[]" value="<?=$telephone[$k]?>"/>
                </td>
                <?php 
                if($k>0){
                ?>
        ...
        <tr>
          ...
          <td width="92%" colspan="3" class="cknr2">
            <input type="text" class="bzsr" name="mark" value="<?=$mark?>" />
          </td>
        </tr>
...
function save(){
	...
	$.post(
		...
		function (data) //回传函数
		{
			...
			if(data=='操作成功')
			{
				location.href="/index.php/investment/view/"+id+'/'+thispage;
			}
		}
	);
}
