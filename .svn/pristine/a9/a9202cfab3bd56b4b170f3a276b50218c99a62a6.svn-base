<div class="maincon">
  ...
  <div style="width: 80%; margin: 120px auto; padding-left: 50px; left:auto;position:fixed;">
	...
	<div class="caozuo8">
		...
		<input type="button" class="b_bnt01" value="编 辑" onClick="window.location.href='/index.php/contact/edit/<?=$id?>/<?=$thispage?>'" />
	  ...
	  <tr> 
	    <td colspan="4">
		  <table cellspacing="0" cellpadding="0" class="biaozhun2">
			...
			<tr>
				<td width="4%" class="cklt">分 组：</td>
				<td width="20%" class="cknr2"><?=$v['type_name']?></td>
				<td width="6%" class="cklt">单位名称：</td>
				<td width="16%" class="cknr2"><?=$v['company_name']?></td>
				<td width="6%" class="cklt">单位网站：</td>
				<td width="10%" class="cknr2"><?=$v['url']?></td>
				<td width="6%" class="cklt">单位邮编：</td>
				<td width="8%" class="cknr2"><?=$v['postcode']?></td>
				<td width="4%" class="cklt">职 务：</td>
				<td width="10%" class="cknr2"><?=$v['position']?></td>
                <td width="6%" class="cklt">单位落户：</td>
				<td width="8%" class="cknr2"><?=$v['settlename']?></td>
            </tr>
<!--			<tr>-->
<!--				<td width="7%" class="cklt">单位网站：</td>-->
<!--				<td width="18%" class="cknr2">--><?//=$v['url']?><!--</td>-->
<!--				<td width="7%" class="cklt">单位邮编：</td>-->
<!--				<td width="28%" class="cknr2">--><?//=$v['postcode']?><!--</td>-->
<!--				<td width="7%" class="cklt"></td>-->
<!--				<td width="15%" class="cknr2"></td>-->
<!--				<td width="7%" class="cklt"></td>-->
<!--				<td width="10%" class="cknr2"></td>-->
<!--			</tr>-->
			<?php endforeach?>
          </table>
		</td>
	  ...
	  <tr>
		...
		<td width="92%" class="cknr2">
                <?php if(is_array($fax)):?>
				<?php foreach($fax as $v) :?>
					<p class="hmyjjh" ><?=$v?>；</p>
				<?php endforeach?>
			<?php else:?>
				<p class="hmyjjh" ><?=$fax?></p>
			<?php endif?>
                </td>
	  </tr>
	  <tr id="dzxx"> 
	  ...
	  <tr>
	    <td width="8%" class="cklt">E-mail：</td>
            <td width="92%" colspan="3" class="cknr2">
			<?php if(is_array($email)):?>
				<?php foreach($email as $v) :?>
					<p class="hmyjjh" ><?=$v?>；</p>
				<?php endforeach?>
			<?php else:?>
				<p class="hmyjjh" ><?=$email?></p>
			<?php endif?>
		</td>
	  </tr>
	  <tr> 
		<td width="8%" class="cklt">名 片：</td>
		<td colspan="3" class="cknr2"><p class="hmyjjh" ><a href="<?=$pic_front==''?'/images/logo_03.jpg':$pic_front?>" target="_blank"><img src="<?=$pic_front==''?'/images/logo_03.jpg':$pic_front?>" id="photo"/></a><a href="<?=$pic_reverse==''?'/images/logo_03.jpg':$pic_reverse?>" target="_blank"><img src="<?=$pic_reverse==''?'/images/logo_03.jpg':$pic_reverse?>" id="photo" class="mor_pic"/></a></p><input type="button" value="+" class="tj_bnt tabn" id="add_pic" style=" position: relative; top: 10px; "/><input type="button" value="-" class="tj_bnt tabn" id="min_pic" style=" display: none; position: relative; top: 10px; "/></td>
	  </tr>
	  <tr> 
