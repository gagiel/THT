<?php
class M_remind_copy extends CI_Model {

    var $table_name = 't_remind_copy';
...
//        $this->db->like("concat(',',range_user,',')",",$userid,");
//        $this->db->order_by('addtime','desc');
//        $re = $this->db->get($this->table_name);
//        $plan_list=$re->result();
//        //工作方案权限判断
//        foreach($plan_list as $key=>$v){
//            if(in_array($userid,explode(",",$v->range_user_do))){
//                unset($plan_list[$key]);
//            }
//        }
//        return $plan_list;
    ...
    function remind_read($data)
    {
        ...
        $re = $this->db->get('t_remind_read_copy');
        $data['addtime'] = date('Y-m-d H:m:s');
        if($re->num_rows)
        {
            $this->db->where('remind_id',$data['remind_id']);
            $this->db->where('user_id',$data['user_id']);
            return $this->db->update('t_remind_read_copy',$data);
        }
        else
        {
            return $this->db->insert('t_remind_read_copy',$data);
        }
    }
    ...
    {
        $this->db->select('t_remind_read_copy.state,t_user.name,t_remind_read_copy.addtime');
        $this->db->where('remind_id',$id);
        $this->db->join('t_user', 't_user.id=t_remind_read_copy.user_id', 'left');
        $re = $this->db->get('t_remind_read_copy');
        return $re->result();
    }
}
