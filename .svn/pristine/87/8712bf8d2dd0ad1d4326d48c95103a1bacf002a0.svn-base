...
<!--	<input type="button" class="b_bnt01" value="删 除" onClick="delInfo()" />-->
	<form id="get_form" action="/index.php/plan_r/index" method="post" style="float: right;">
	...
	?>
	  ...
	  <tr>
		...
		<input type="button" value="准备" class="s_bnt01 green" onClick="editReady(<?=$v->id?>)" />
		  ...
		  <td width="6%">
			  <?php if(in_array($user_id,explode(",",$v->canyu_id)) ||$user_id==$v->fabu_id || in_array('30',explode(",",$jurisdict))){?>
			  <input type="button" value="修改" class="s_bnt01 green" onClick='editInfo(<?=$v->id?>)' />
			  <?php }?>
			  <?php if($user_id==$v->fabu_id || in_array('30',explode(",",$jurisdict))){?>
			  <input type="button" value="删除" class="s_bnt01 red" onClick='delInfo(<?=$v->id?>)' />
			  <?php }?>
		  </td>
...
function choose_plan(){
	...
	if(i=='1'){
		location.href="/index.php/plan_r/add";
	}else if(i=='2'){
		location.href="/index.php/plan_r/add_more";
	}else{
		alert('没有选择方案类型');
	}
	closeWin();
...
{
	location.href="/index.php/plan_r/edit_1/"+id;
}
