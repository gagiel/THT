...
?>
    <div class="maincon">
        ...
        <div class="caozuo10">
            ...
            <form id="get_form" action="/index.php/user/register_list" method="post" style="float:right;">
                ...
                if(is_array($list))
                {
                    ...
                    foreach($list as $v)
                    {
                        ...
                        <tr class="tab_content" id="tr_<?=$v->id?>_<?=$v->mark?>">
                            ...
                            <td><?php if($v->reviewed=='0'){echo '等待审核';}else if($v->reviewed=='1'){echo '审核已通过';}else if($v->reviewed=='2'){echo '审核撤销';}?></td>
                            <? if(in_array('3',$arr_jurisdict)){ ?>
                                <td>
                                    ...
                                    <?php if($v->reviewed==0 || $v->reviewed==2){?>
                                    <input name="" type="button" value="审核" class="s_bnt01 red" onClick="reviewed(<?=$v->id?>)"/>
                                    <?php }else{?>
                                        <input name="" type="button" value="撤销" class="s_bnt01 red" onClick="cancel_reviewed(<?=$v->id?>)"/>
                                    <?php }?>
...
<? if(in_array('3',$arr_jurisdict)){ ?>
    ...
    <div class="newli" id="winregister">
        ...
        <form id='sub_form' action="/index.php/user/add" method="post" >
            ...
            <input type="hidden" name="register_type" value="yes">
            <div class="nl_det">
        ...
        <form id='tree_form' action="/index.php/user/jurisdict_register" method="post">
    ...
    <script>
        ...
        function editInfo(i)
        {
            ...
            $('#sub_form').attr('action', '/index.php/user/update_register');
            $('#u_id').val($('#id_'+i).val());
        ...
        function cancel_reviewed(id)
        {
            if(confirm("确认撤销审核吗？"))
            {
                $.post(
                    "/index.php/user/reviewed_cancel/",
                    {
                        id:id
                    },
                    function (data) //回传函数
