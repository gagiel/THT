...
class Wechat extends CI_Controller
{
	...
	var $token = "thtprm";
	var $appId = "wxe5faea4c135ec9a0";
	var $appSecret = "0d51f4efbec6a033aac28eef7ab32480"; 
	...
	public function jyxc()
	{
		...
		$re = $this->m_wechat_set->jyxc_list($where);
		...
		if(is_array($re))
		{
			...
			if(is_array($userList))
			{
				...
				{
					$arr_user[$v->id] = $v->name;
				}
			}
			
			foreach($re as $k => $v)
			{
				if($v['re_user']!='')$re[$k]['re_uname'] = $arr_user[$v['re_user']];
				if(isset($arr_openid[$v['from_user']]))
				{
					$re[$k]['nikename'] = $arr_openid[$v['from_user']];
				}
				else 
				{
					$url = "https://api.weixin.qq.com/cgi-bin/user/info?access_token=" . $access_token . "&openid=" . $v['from_user'] . "&lang=zh_CN";
					$ch = curl_init(); 
					...
					$nickname = $arr_tmp->nickname;
					
					$re[$k]['nikename'] = $nickname;
					$arr_openid[$v['from_user']] = $nickname;
				}
			}
		}
		...
		$this->load->view('wechat/jyxc_list',$data);
		$this->load->view('main_footer');
	}
	 ...
	 * 建言献策回复
	 */
	public function jyxc_re()
	{
		$this->load->library('session');
		$this->load->library('login');
		$this->login->check_login($this->session->userdata('user_id'),true);
		$this->login->check_jurisdict('17',$this->session->userdata('user_jurisdict'),true);
		
		$id = $this->input->post('id');
		$now = time();
		$data = array(
			're_info' => $this->input->post('info'),
			're_user' => $this->session->userdata('user_id'),
			're_time' => $now
		);
		
		if($this->m_wechat_set->re_jyxc($id,$data))
		{
			exit('success|'.$this->session->userdata('user_name').'|'.date('Y-m-d H:i:s'));
		}
		else 
		{
			exit("回复失败，请重试");
...
//		$pic = file_get_contents('/images/weixin_newsletter_2015.jpg');
	...
	public function addThumbMedia($path=false)         
	{
         ...
         * 上传图文消息内的图片获取URL
         */                         
        public function uploadimgLogo($path){                                  
              $access_token = $this->get_access_token();
		if(!$access_token)
		{
			exit("微信接口验证失败");
		...
		$output = $this->https_request($url,$filedata);
            ...
            /**
             * 提取字符串中图片url地址并对地址进行处理
             * @param type $str
             * @return type
             */
            function replaceImgsByStr($str) {
                ...
                foreach ($matches[0] as $value) {
                    $data[]= $this->uploadimgLogo($value);
                }
                $restr=str_replace($matches[0], $data, $str); 
                return $restr;   
                }else{
                return $str;    
                }
	...
	public function sendNewsLetter() {
		    ...
		    $content['data'] = str_replace('src="', 'src="http://prm.tht.gov.cn', $content['data']);
                    //图文中的图片上传，并重新写入图文信息中
                    $content['data']=$this->ReplaceImgsByStr($content['data']);
		    $tmpdata['content'] = urlencode(htmlspecialchars(str_replace("\"", "'", str_replace("\n", "<br>", $content['data']))));
		    $tmpdata['digest'] = urlencode(htmlspecialchars(str_replace("\"", "'", $data['description'])));
		    $tmpdata['show_cover_pic'] = 1;
		    $tmp[] = $tmpdata;
			...
			foreach ($other_data as $value) {
			    ...
			    $value['content'] = str_replace('src="', 'src="http://prm.tht.gov.cn', $value['content']);
                             //图文中的图片上传，并重新写入图文信息中
                            $value['content']=$this->ReplaceImgsByStr( $value['content']);
	...
	public function responseMsg()
	{
		...
		elseif($this->msgType=='text')
		{
			...
			//建言献策回复查询
			if(substr($this->content,0,6)=='JYXCRE')
			{
				$this->getJYXCRE();
			}
		}
			...
			'button'	=> array(
				...
				array(
					...
					'sub_button'=>array( 
						...
						array('type'=>'view','url'=>'http://prm.tht.gov.cn/appfile/android.html',	'name'=>urlencode("安装安卓版"),),
						array('type'=>'view','url'=>'https://itunes.apple.com/cn/app/tian-jin-gao-xin-qu-gong-gong/id964430108?l=en&mt=8',	'name'=>urlencode("安装IOS版"),),
					),
				),
			),
		);
	...
	public function reNewsLetterMonth()
	{
		...
		{			
			...
			$this->reNews(array($re_data));
				...
				"description"	=> "点击查看月刊内容",
				"picurl"		=> "http://prm.tht.gov.cn/images/weixin_newsletter_2015.png",
				"url"			=> "http://prm.tht.gov.cn".$ninfo->url
			);
			$this->reNews(array($re_data)); 
                }
		else
	...
	private function reJYXC()
	{
		...
		do {
			$recode = "JYXCRE".time().rand(100, 999);
			$codeexist = $this->m_wechat_set->check_recode($recode);
		}while($codeexist);
		...
		if($this->m_wechat_set->jyxc_insert($data))
		{
			...
			$redata = array(
			    ...
			    "msgtype"	=> "text",
			    "text"		=> array("content"	=> urlencode("提交成功，我们会尽快回复。您可发送以下查询码，查询回复信息："))
			);
			$jsondata = urldecode(json_encode($redata));

			//调用群发接口，进行群发操作
			
			//$this->reText($jsondata);
			$rejson = $this->https_request($url, $jsondata);
			//$this->reText($rejson);
	...
	private function getJYXCRE()
	{
		$re = $this->m_wechat_set->get_jyxc_re($this->content);
		if($re['succ'])
		{
			if($re['info']=='')
			{
				$this->reText("处理中，我们会尽快回复");
			}
			else 
			{
				$this->reText($re['info']);
