<?php
class M_plan_copy extends CI_Model {

    var $table_name = 't_plan_copy';
     ...
     * plan_list	活动方案列表
     */
    function plan_list($where,$start='0',$pageSize='0')
    {
        $this->db->select('*');
...
//        $this->db->like("concat(',',fabu_id,',')",',1,');
        ...
        foreach($plan_list as $key=>$v){
            if(!in_array($user_id,explode(",",$v->canyu_id)) && $user_id!=$v->fabu_id && !in_array($user_id,explode(",",$v->users))){
                unset($plan_list[$key]);
            }
        }
        return $plan_list;
