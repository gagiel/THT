...
class plan_r extends CI_Controller {
    ...
    function __construct()
    {
        ...
        $this->load->model('m_plan_copy');//工作方案表
        $this->load->model('m_plan_fankui');//工作方案反馈
        $this->load->model('m_plan_note');//操作记录
        $this->load->model('m_plan_templet');//通用模板
        $this->load->model('m_plan_ready_copy');//工作方案前期准备表
        $this->load->model('m_remind_copy');//工作提醒
        $this->load->model('m_plan_annex_copy');
        $this->load->model('m_plan_division_copy');
        $this->load->model('m_plan_done_copy');
...
//		exit($_error->show_error('测试标题', '测试错误标题内容', 'error_to_index'));
        ...
        $this->load->view('main_menu',array('menu'=>'plan_r'));
        ...
        //---------分页------------
        $num = $this->m_plan_copy->plan_num($where);
        ...
        $config['base_url'] = '/index.php/plan_r/index/';
        ...
        //-----根据条件获取列表------
        $list = $this->m_plan_copy->plan_list($where,($page-1)*$pageSize,$pageSize);
        $data['list']	= $list;
...
//            $data['list'][$key]->text=$i;
//            $i++;
//        }
        //管理员权限
        $user_id_j=$this->session->userdata('user_id');
        $where_fankui['u_id']=$user_id_j;
        $j_list=$this->m_user->user_get_r($where_fankui);
        $data['jurisdict']=$j_list[0]['jurisdict'];


        $this->load->view('plan_r/index',$data);
    ...
    public function plan_info($id='0')
    {
        ...
        if($id>0)
        {
            $data = $this->m_plan_copy->plan_get(array('id'=>$id),'arr');
        }
        var_dump($data);
        //右上角查询
        $this->load->helper('select');
    ...
    public function view()
    {
        ...
        //方案内容
        $info = $this->m_plan_copy->plan_get(array('id'=>$id));
    ...
    public function add1()
    {
        ...
        //当前最大编号
        $data['num'] = $this->m_plan_copy->max_num();
    ...
    public function add()
    {
        ...
        //$this->load->view('plan/add_1',$data);
        $this->load->view('plan_r/add_3',$data);
        $this->load->view('main_footer');
    }
    ...
    public function add_more(){
        ...
        //$this->load->view('plan/add_1',$data);
        $this->load->view('plan_r/add_more',$data);
        $this->load->view('main_footer');
    }
    //短信群发列表页
...
//		$data['page']	= $page;
    ...
    public function area_text(){
        if(!empty($_GET['plan_id'])){
            ...
            //内容
            $data['info'] = $this->m_plan_copy->plan_get(array('id'=>$plan_id));
            $num = $data['info']->num;
...
//			}
    ...
    public function edit($id)
    {
        ...
        //内容
        $data['info'] = $this->m_plan_copy->plan_get(array('id'=>$id));

        //准备事项
        //$this->load->model('m_plan_ready');
        $ready=$this->m_plan_ready_copy->plan_ready_list(array('plan_id'=>$id));
        if(is_array($ready))
        {
            foreach($ready as $v)
            {
        ...
        //附件信息
        $file=$this->m_plan_annex_copy->plan_annex_list(array('plan_id'=>$id));
        if(is_array($file))
        {
            $data['file']=$file;
        }
    ...
    public function edit_2($id)
    {
        ...
        //具体安排
        $done=$this->m_plan_done_copy->plan_done_list(array('plan_id'=>$id));
        if(is_array($done))
        {
            $data['done']=$done;
        }
        ...
        //分工内容
        $division=$this->m_plan_division_copy->plan_division_list(array('plan_id'=>$id));
    ...
    public function edit_1($id)
    {
        ...
        //工作方案参与人
        $canyu_id=explode(',',$data['info']->canyu_id);
        if(!empty($canyu_id[0])){
            $u=array();
            foreach($canyu_id as $val)
            {
                foreach($userList as $v)
                {
                    if($val==$v->id)
            ...
            $user=  implode(',', $u);
            $canyu_name_list=array();

            $canyu_name=$this->m_user->user_get_r_2($data['info']->canyu_id);
            foreach($canyu_name as $key=>$val){
                $canyu_name_list[$key]=$val['name'];
            }
            $data['info']->canyu_name = $canyu_name_list;
            $data['info']->canyu_id=$data['info']->canyu_id;
        }
        ...
        $where_fankui['u_id']=$user_id;
        $fankui_list=$this->m_user->user_get_r($where_fankui);
        $data['fankui_name']=$fankui_list[0]['name'];
        $data['jurisdict']=$fankui_list[0]['jurisdict'];
        //工作反馈list
        $data['fankui_list']=$this->m_plan_fankui->plan_list($id);

        $this->load->view('plan_r/edit_1',$data);
        $this->load->view('main_footer');
    }
    //工作提醒右下角弹出框ifram
    public function work_tixing(){
        $userid=$this->session->userdata('user_id');
        $list = $this->m_remind_copy->get_remind_by_userid($userid);
        $data['list']=$list;
        $this->load->view('plan_r/work_tixing',$data);
    }
    //工作提醒左上角列表内容ajax回传数据
    public function work_tixing_top(){
        $userid=$this->session->userdata('user_id');
        $list = $this->m_remind_copy->get_remind_by_userid($userid);
        $data['list']=$list;
        $html="";
        foreach($list as $v) {
            $html .= "<li value=''><p><span id='remind_' style='color:gray'>内容</span></p>
            <div class='read'><a onclick='read(this,1)'>已阅</a></div>
    ...
    public function work_tixing_status(){
        $userid=$this->session->userdata('user_id');
        $list = $this->m_remind_copy->get_remind_by_userid($userid);
        $data['list']=$list;
        if(!empty($list)){
...
//			$msg=$_POST['mass_area'];
    ...
    public function pushmessage(){
        ...
        $user = $_POST['user'];
        $newData['users']		= $user;
        $re = $this->m_plan_copy->plan_update($id,$newData);
        $this->m_plan_copy->plan_state($id,1);  //发布
        $subject = $_POST['text'];
        $senstr=$this->sensitivestr->filter_SensitiveStr($subject);
        if($senstr!=""){
            echo '发送失败！短信内容含有敏感字符：“'.$senstr.'”,请修改后重新发送。';
    ...
    {
//		print_r($this->input->post('c_plan_more'))."<br>";
//		print_r($this->input->post('c_time_start'))."<br>";
//		print_r($this->input->post('c_time_end'))."<br>";
//		print_r($this->input->post('c_time_xi'))."<br>";
//		print_r($this->input->post('c_plan_more_xi'))."<br>";
//		die();
        $canyu_id   = $this->input->post('canyu_id');//参与人id集合
        $title		= $this->input->post('c_title');
        $c_num          =$this->input->post('c_num_n');
        $num		= $this->input->post('c_num_y').'-'.$c_num;
        $affairs	= $this->input->post('c_affairs');
        ...
        $templet_id  =$this->input->post('templet_id');
        ...
        //方案模板
        $templet = $this->m_plan_templet->plan_templet_get(array('id'=>$templet_id));
        ...
        if(!empty($info_title_moban[0])){
            foreach($info_title_moban as $key=>$val){
                if($key=="0"){
                    $info_title_moban[$key]="附件: ".$val;
                }else{
                    $info_title_moban[$key]="&nbsp;&nbsp;&nbsp;".$val;
                }
            }
        }
            ...
            for($k=0;$k<count($arr);$k++)
            {
                ...
                switch($v)
                {
                    ...
                    case 'for':
                        ...
                        for($j=$k;$j<count($arr);$j++)
                        {
                            if(in_array($arr[$j],array('address','name','department','plan','done','info_title_moban','people_name_moban')))
                            {
                                $type = $arr[$j];
                                break;
                            }
...
//							}
                            ...
                            if(!empty($plan[0])){
                                ...
                                $str.="
									<table width='100%' style='border:1px solid rgb(255,255,255)'>";
                                foreach($plan as $key=>$val){
                                    $str.="<tr style='border:1px solid rgb(255,255,255)'>
                            ...
                            if(!empty($c_plan_more)){
                                ...
                                foreach($c_plan_more as $key=>$val){
                                    if($c_time_start[$key]!=$c_time_end[$key]){
                                        $str.="<p style='font-family: 仿宋_GB2312; font-size: 22px;'><strong>$c_time_start[$key]-$c_time_end[$key]</strong></p>";
                                        $str.="<p style='font-family: 仿宋_GB2312; font-size: 22px;'>　　$c_plan_more[$key]</p>";
                                    }else{
                                        $str.="<p style='font-family: 仿宋_GB2312; font-size: 22px;'><strong>$c_time_start[$key]</strong></p>";
                                        $str.="<p style='font-family: 仿宋_GB2312; font-size: 22px;'>　　$c_plan_more[$key]</p>";
                                    }
                                    if(!empty($c_plan_more_xi[$key])){
                                        foreach($c_plan_more_xi[$key] as $key1=>$val1){
                                            $str.="<table width='100%' style='border:1px solid rgb(255,255,255)'>";
                                            $str.="<tr style='border:1px solid rgb(255,255,255)'>";

                                            if(!isset($c_time_xi[$key][$key1]{1})){
                                                $str.="<td valign='top' style='border:1px solid rgb(255,255,255);width:60px;text-align: left'><p style='font-family: 仿宋_GB2312; font-size: 22px;'>".$c_time_xi[$key][$key1]."</p></td>";
                                            }else{
                                                $str.="<td valign='top' style='border:1px solid rgb(255,255,255);width:60px;text-align: right'><p style='font-family: 仿宋_GB2312; font-size: 22px;'>".$c_time_xi[$key][$key1]."</p></td>";
                                            }
                                            $str.="<td style='border:1px solid rgb(255,255,255);text-align: left'><p style='font-family: 仿宋_GB2312; font-size: 22px;'>"."　".$c_plan_more_xi[$key][$key1]."</p></td>
                        ...
                        if($type=='info_title_moban' &&  count($info_title_moban)>0)
                        {
                            $n = 0;
                            for($i=0;$i<count($info_title_moban);$i++)
                            {
                                if($info_title_moban[$i]=='')
                                {
                                    continue;
                                }
                                if($n>0)
                                ...
                                $k1 = $k+1;
                                do{
                                    if($arr[$k1]=='info_title_moban')
                                    {
                                        $str .= $info_title_moban[$i];
                                    }
                                    elseif($arr[$k1]=='i')
                                    {
                                        $str .= $n;
                        ...
                        if($type=='people_name_moban' &&  count($people_name_moban)>0)
                        {
                            $n = 0;
                            for($i=0;$i<count($people_name_moban);$i++)
                            {
                                if($people_name_moban[$i]=='')
                                {
                                    continue;
                                }
                                if($n>0)
                                ...
                                $k1 = $k+1;
                                do{
                                    if($arr[$k1]=='people_name_moban')
                                    {
                                        $str .= $people_name_moban[$i];
                                    }
                                    elseif($arr[$k1]=='i')
                                    {
                                        $str .= $n;
...
//		if(!empty($people_name[0])){
//			$str.="<p style='font-family: 仿宋_GB2312; font-size: 22px;'></p>
//					<table width='100%' style='border:1px solid rgb(255,255,255)'>";
//			foreach($people_name as $key=>$val){
//			$str.="<tr style='border:1px solid rgb(255,255,255)'>
//							<td style='border:1px solid rgb(255,255,255);text-align: left;width:120px'><p style='font-family: 仿宋_GB2312; font-size: 22px;'>";
//						if($key=='0'){
//							$str.="联系人：";
//						}
//							$str.="</p></td>
//							<td style='border:1px solid rgb(255,255,255);text-align: left;width:120px'><p style='font-family: 仿宋_GB2312; font-size: 22px;'>$val</p></td>
//							<td style='border:1px solid rgb(255,255,255);text-align: right;width:120px'><p style='font-family: 仿宋_GB2312; font-size: 22px;'>联系电话:</p></td>
//							<td style='border:1px solid rgb(255,255,255);text-align: left;'><p style='font-family: 仿宋_GB2312; font-size: 22px;'>$people_phone[$key]</p></td>
//					</tr>";
//			}
//			$str.="</table>";
//		}
        ...
        //发布人的id
        $fabu_id=$this->session->userdata("user_id");
        $data = array(
            'num'		=> $num,
            'title'		=> $title,
            'affairs'	=> $affairs,
        ...
        if($id==0)
        {
            ...
            $re = $this->m_plan_copy->plan_insert($data);


            //var_dump($re);
            if($re){
                $list=$this->m_plan_copy->plan_get(array('num'=>$num));
                $planid=$list->id;
                if(!empty($ready)){
                    ...
                    foreach($temp as $v){
                        $this->m_plan_ready_copy->plan_ready_insert($v);
                    }
                }
                if(!empty($file)){
                    $filelist=array();
                    ...
                    foreach($filelist as $v){
                        $this->m_plan_annex_copy->plan_annex_insert($v);
                    }
                }
                if(!empty($done)){
                    $divisionlist=array();
                    ...
                    foreach($divisionlist as $v){
                        $this->m_plan_division_copy->plan_division_insert($v);
                    }
                }
                //增加多日活动安排入库
                $c_plan_more=$this->input->post('c_plan_more');
                ...
                if(!empty($c_plan_more)){
                    ...
                    $this->m_plan_done_copy->plan_done_insert($done_more);
                ...
                if(!empty($plan)){
                    ...
                    foreach($donelist as $v){
                        $this->m_plan_done_copy->plan_done_insert($v);
                    }
                }
            }
            ...
            $data['c_num']		= $c_num;
            //百度坐标
            $re = $this->m_plan_copy->plan_update($id,$data);
            if($re){
                //准备相关处理
                $this->m_plan_ready_copy->plan_ready_delete_by_planid($id);
                $this->m_plan_annex_copy->plan_annex_delete_by_planid($id);
                $this->m_plan_division_copy->plan_division_delete_by_planid($id);
                $this->m_plan_done_copy->plan_done_delete_by_planid($id);
                if(!empty($ready)){
                    foreach($ready as $key=>$value){
                        $temp[$key]['plan_id']=$id;
                        $temp[$key]['info']=$value;
        ...
        if($re)
        {
            ...
            $note=array(
                ...
                'name'=>$data['fankui_name'],
                'type'=>'1',
                'time'=>time(),
                'plan_id'=>$id
            );
            $this->m_plan_note->plan_insert($note);
            if($id){
                echo '<script>location.href="/index.php/plan_r/edit_1/'.$id.'";</script>';
                if(!empty($_SESSION['plan_area'])){
                    unset($_SESSION['plan_area']);
                }
            }else{
                echo '<script>location.href="/index.php/plan_r/edit_1/'.$planid.'";</script>';
                if(!empty($_SESSION['plan_area'])){
                    unset($_SESSION['plan_area']);
                }
            }
    ...
    public function fankui(){
        $_POST['addtime']=time();
        $re = $this->m_plan_fankui->plan_insert($_POST);

        if($re){
            $re = $this->m_plan_fankui->plan_list($_POST['plan_id']);
            echo "<table>";
    ...
    public function save()
    {
        ...
        if($id==0)
        {
            ...
            $re = $this->m_plan_copy->plan_insert($data);
            if($re){
                $list=$this->m_plan_copy->plan_get(array('c_num'=>$data['c_num']));
                $planid=$list->id;
                if(!empty($ready)){
                    foreach($ready as $value){
                        ...
                        $temp['info']=$value;
                        $temp['r_state']=0;
                        $this->m_plan_ready_copy->plan_ready_insert($temp);
                    }
                }
                if(!empty($file)){
                    $filelist=array();
            ...
            if(!empty($del_ready)){
                foreach($del_ready as $value){
                    $this->m_plan_ready_copy->plan_ready_delete($value);
                }
            }
            if(!empty($ins)){
            ...
            if(!empty($upd)){
                foreach($upd as $value){
                    $temp['info']=$value['info'];
                    $this->m_plan_ready_copy->plan_ready_update($value['id'],$temp);
                }
            }
...
//                        }
    ...
    public function state()
    {
        ...
        echo $this->m_plan_copy->plan_state($id,$state);
    }
...
//        $idArray = explode(',',$id);
        $data = array('isdel'=>1);
        //此处为假删除
        $update_status = $this->m_plan_copy->plan_update_by_id($id,$data);
        //删除活动方案准备
    ...
    public function selectReady(){
        $id=$this->input->post('id');
        $this->load->model('m_plan_ready_copy');
        $ready=$this->m_plan_ready_copy->plan_ready_list(array('plan_id'=>$id));
    ...
    public function saveReady(){
        ...
        if(!empty($ready_0)){
            foreach($ready_0 as $v){
                $temp['r_state']=0;
                $temp['r_time']=time();
                $this->m_plan_ready_copy->plan_ready_update($v,$temp);
            }
        }
        if(!empty($ready_1)){
    ...
    public function saveTxUsers(){
        ...
        $data['users']		= $user;
        $re = $this->m_plan_copy->plan_update($id,$data);
        $this->m_plan_copy->plan_state($id,1);
        if($re){
            echo '操作成功';
        }else{
            echo '操作失败';
    ...
    public function exportPlan(){
        ...
        foreach ($resultPart as $key => $value) {
            $data = array(
                'num'=>date('Y').'-'.sprintf("%03d", $this->m_plan_copy->max_num()),
                ...
                'c_num'=>$this->m_plan_copy->max_num(),
    ...
    */
    ...
    public function getPlanInfo($id){
        ...
        //获取方案基本信息
        $plan=$this->m_plan_copy->plan_get(array('id'=>$id));
        $done=$this->m_plan_done_copy->plan_done_list(array('plan_id'=>$id));
        $division=$this->m_plan_division_copy->plan_division_list(array('plan_id'=>$id));
