	...
	<table cellpadding="0" cellspacing="0" class="biaozhun">
	  <tbody>
	    <tr class="tab_tit alt">
	      <td width="20%">用户发送内容</td>
	      <td width="10%">回复类型</td>
	      <td width="30%">回复内容</td>
	      <td width="10%">查看页</td>
	      <td width="18%"><input name="name" value="新增" class="s_bnt01" onclick="addInfo()" type="button"></td>
	    </tr>
<?
if(is_array($list))
{
...
?>
	    ...
	    <tr class="alt">
	      <td id="get_msg_<?=$i?>"><?=$v->get_msg?></td>
	      <td id="re_type_<?=$i?>"><?=$v->re_type?></td>
	      <td id="re_msg_<?=$i?>"><?=$v->re_msg?></td>
	      <td><? if($v->view=='')echo '&nbsp;';else{?><a href="/index.php/wechat/view/<?=$v->get_msg?>" target="_blank">点击查看</a><? }?></td>
	      <td>
