  ...
  <div class="con_detail">
   <div style="margin:10px; min-width:900px;">
    ...
    <div style="float:left; min-width:490px;">
	    ...
	    <input type="hidden" class="bzsr" id="c_num" name="c_num" value="<?=$info['c_num']?>"/>
	    ...
	    <input type="text" class="Wdate" id="time" name="time" width="120" value="<?=$info['start']?>" onFocus="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm'})"/>
	   <p class="szts"><span></span></p>
	    <label class="sizi">出席领导：</label>
	    <input type="hidden" class="bzsr" id="name" name="name" width="120" value="<?=implode(",", $info['users'])?>"/>
            <lable id="names_show" style="float:left;line-height:36px;margin: 0 5px;"><?=implode(",", $info['users'])?></lable>
            <input class="btn_cxld" value="请选择"  type="button" onclick="get_ld()">
	    <p class="szts"><span></span></p>
	    <label class="sizi">参加范围：</label>
	    <input type="hidden" class="bzsr" id="department" name="department" width="120" value="<?=implode(",", $info['department'])?>" />
	    <lable id="department_show" style="float:left;line-height:36px;margin: 0 5px;"><?=implode(",", $info['department'])?></lable>
        ...
        if(is_array($d_list))
        {
          ...
          <li class="Closed">
            ...
            <li>
              ...
              <input type="checkbox" name="range_user" id="u_<?=$j?>" value="<?=$u->id?>" class="check_<?=$i?>"<?=in_array($u->name,$users)?' checked':''?> />
	...
	<p class="szts"><span></span></p>
	  ...
	  <tr>
	       ...
	       foreach($info['ready'] as $k=>$v ){
	    ?> 
                  <tr>
                    <td>
			<input class="bzsr11" name="ready[]" type="text"  value="<?=$v?>"/>
            ...
            <td colspan="4">
              <table class="biaozhun04" cellpadding="0" cellspacing="0">
                <tbody>
                  <tr class="tab_tit alt">
                   ...
                   <?php
                    ...
                    <tr >
			   <td width="80%">
                               <input class="bzsr11" name="c_file[]" type="text" style="width:95%;" id="file_<?=$key?>" value="<?=$value?>"/>
                               <input class="bzsr11" name="c_fileurl[]" type="hidden" id="fileurl_<?=$key?>" value="<?=$info['fileurl'][$key];?>"/>
			   </td>
                           <td width="10%">
			   ...
			   <td width="80%">
                               ...
                               <input class="bzsr11" name="c_fileurl[]" type="hidden" id="fileurl_0" value=""/>
			   </td>
                           <td width="10%">
	...
	<textarea name="other" cols="" rows="" id="other" class="bzsr13"><?=$info['other']?></textarea>
	<p class="szts"><span></span></p>
   </div>
  </div>
  <div class="caozuo5">
...
/* 添加前期准备 */
	$("#add_ready").click(function(){
		var tr = '<tr>'
			   +   '<td>'
			   +     '<input class="bzsr11" name="ready[]" type="text"/>'
			   +   '</td>'
			   +   '<td><input type="button" value="-" class="tj_bnt tabn" style=" margin-left: 8px;" onclick="del_ready(this)"/></td>'
			   + '</tr>';
