...
<div class="maincon">
   ...
   <div style="margin:10px 0 0 50px; min-width:900px;">
     <form id="create_form" name="create_form" method="post" action="/index.php/event/save" >
	    ...
	    ?>
	     ...
	     ?>
	       <input type="radio"  name="date_type" value="1" />年-月	
	       <input type="radio"  name="date_type" value="2" checked />年-月-日
	    <?php 
	    }
	    ?>
	    </span>
	    ...
	    if($date_type==1){
	    ?>
	    <input type="text" class="Wdate" id="time" name="time" width="120" onFocus="WdatePicker({dateFmt:'yyyy-MM'})" value="<?=date('Y-m',$time)?>" />
	    <input type="text" class="Wdate" id="time1" name="time" width="120"  onFocus="WdatePicker({dateFmt:'yyyy-MM-dd'})" style="display:none" />
	     <?php 
	    }else if($date_type==2){
	     ?>
	    <input type="text" class="Wdate" id="time" name="time" width="120"  onFocus="WdatePicker({dateFmt:'yyyy-MM'})" style="display:none" />
	    <input type="text" class="Wdate" id="time1" name="time" width="120"  onFocus="WdatePicker({dateFmt:'yyyy-MM-dd'})" value="<?=date('Y-m-d',$time)?>"  />
	    <?php 
	    }
	    ?>
	    <p class="szts"><span></span></p>
	    <label class="sizi">标题：</label>
	    <textarea name="title" cols="" rows="" class="bzsr13" id="title"><?=$title?></textarea>
	   	<p class="szts"><span></span></p>
        ...
        <input type="radio"  name="is_link" value="1" checked />是
	    <?php 
	    }
	    ?>
        </span>
	    ...
	    <input type="text" class="bzsr"  style="width:300px" name="url" value="<?=$url?>"/>
	    <p class="szts"><span></span></p>
	    <label class="sizi">内容：</label>
	   	<div style="margin-left:90px">
			    <!-- 加载编辑器的容器 -->
				<script id="info"><?=$content?></script>
				<!-- 配置文件 -->
  ...
  <div class="caozuo5">
    ...
    <input type="button" id="btn_back" class="b_bnt01" value="返 回" />
  </div>
</div>
</div>
		...
		$.post(
			...
			function (data) //回传函数
			{
				...
				if(data=='操作成功')
				{
					...
					location.href="/index.php/event/index";
				}
			}
		);
	});
