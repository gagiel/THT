  ...
  <div class="con_detail">
   <div style="margin:10px; min-width:900px;">
    <div class="info_left" style="float:left; overflow:auto;">
	    ...
	    <input type="text" class="bzsr" id="num" name="num" value="<?=$num?>"/>
	    <p class="szts"><span></span></p>
	    
	    <label class="sizi">标 题：</label>
	    <input type="text" class="bzsr" id="title" name="title" value="<?=$title?>"/>
	    <p class="szts"><span></span></p>
	    
	    <label class="sizi">时 间：</label>
	    <input type="text" class="Wdate" id="time" name="time" width="120" value="<?=substr($start,0,-3)?>" onFocus="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm'})"/>
	    <p class="szts"><span></span></p>
	    
	    <label class="sizi">出席领导：</label>
	    <input type="hidden" class="bzsr" id="name" name="name" width="120" value="<?=$join_user?>"  />
        <lable id="names_show" style="float:left;line-height:36px;margin: 0 5px;"><?=$join_user?></lable>
        <input class="btn_cxld" value="请选择"  type="button" onclick="get_ld()">
	    <p class="szts"><span></span></p>
	    
	    <label class="sizi">参加范围：</label>
	    <input type="hidden" class="bzsr" id="department" name="department" width="120" value="<?= $join_dept?>" />
        <lable id="department_show" style="float:left;line-height:36px;margin: 0 5px;"><?= $join_dept?></lable>
        ...
        <select class="bzsr2" id="nature" name="nature" >
			...
			if(is_array($plan_nature))
			{
				...
				{
            ?>
            <option value="<?=$key?>" <?php if($key==$nature) echo "selected";?>><?=$value?></option>
			<?
				}
			}
			?>
        ...
        <tbody>
        	<tr class="tab_tit alt">
		  	...
		  	?> 
            <tr>
            	<td><input class="bzsr11" name="ready[]" type="text" tab="<?=$k?>" value="<?=$v?>" /></td>
			...
			<?php
		  	...
		  	?>
            <tr id="last_ready"><td colspan="3" style="height:0px;"></td></tr>
        </tbody>
        </table>
        <p class="szts"><span></span></p>
        ...
        <tbody>
        	<tr class="tab_tit alt">
        		<td>附件信息</td>
        		<td></td>
        		<td width="10%"><input id="add_file" value="+"   class="tj_bnt" style=" float: right; margin-right: 3px; position: relative; top: -2px; " type="button"/></td>
        	</tr>
        	<?php
        	if(!empty($file))
        	{
            ...
            <tr >
            	...
            	<td width="10%">
            		<input type="button" value="-" class="tj_bnt tabn plus" style=" margin-left: 8px;" onclick="del_file(this)"/>
            	</td>
            </tr>
            <?php
        		}
        	...
        	?>
        	<tr id="last_file"><td colspan="3" style="height:0px;"></td></tr>
        </tbody>
        </table>
        <p class="szts"><span></span></p>
        
        <label class="sizi">备注：</label>
        <textarea name="other" cols="20" rows="3" id="other" ><?=$remark ?></textarea>
        <p class="szts"><span></span></p>
